...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/products') {
      ...
      const products = db.products.filter((product) => (!query || `${product.name} ${product.category} ${product.location}`.toLowerCase().includes(query)) && (!category || category === 'All' || product.category === category))
      return send(response, 200, { products })
    }
    if (request.method === 'POST' && url.pathname === '/api/auth/register') {
      const input = await body(request)
      if (!input.name || !input.email || !input.password) return send(response, 400, { error: 'Name, email, and password are required.' })
      if (db.users.some((user) => user.email === input.email.toLowerCase())) return send(response, 409, { error: 'An account with that email already exists.' })
      const user = { id: id(), name: input.name, email: input.email.toLowerCase(), phone: input.phone || '', password: hashPassword(input.password), token: id(), role: 'user', createdAt: new Date().toISOString() }
